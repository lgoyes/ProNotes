...
class InsertLabelInteractor(
    mSubscribeOnScheduler: Scheduler,
    mObserveOnScheduler: Scheduler,
    private val localStorageRepository: ILocalStorageRepository
): ObservableUseCase<Long, Label>(mSubscribeOnScheduler, mObserveOnScheduler) {
    override fun buildUseCase(params: Label): Observable<Long> =
        localStorageRepository.insertLabel(params)
}